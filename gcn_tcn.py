...
class Config:
    ...
    def FULL_SAVE_PATH(self):
        ...
        return os.path.join(self.SAVE_DIR, f"model_GCN_TCN_{self.HORIZON}steps.pth")
...
class GraphConvLayer(nn.Module):
    """
    Symmetric Normalized Graph Convolution Layer (GCN)
    """
...
class TCNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.causal_padding > 0:
            out = out[:, :, :-self.causal_padding]     # Cắt phần tương lai -> causal
...
class GCN_TCN_Model(nn.Module):
    """
    GCN + TCN Model: Graph Convolution + Stacked Dilated Causal Convolution
    """
    def __init__(self, num_nodes, in_feat, gcn_hidden, tcn_channels,
                 tcn_kernel, horizon, output_feat, A_norm=None, dropout=0.3):
        super().__init__()
        self.horizon = horizon
        self.output_feat = output_feat

        # Spatial: 2 lớp GCN
        self.gcn1 = GraphConvLayer(in_feat, gcn_hidden)
        self.gcn2 = GraphConvLayer(gcn_hidden, gcn_hidden)
        self.dropout = nn.Dropout(dropout)

        # Temporal: Stacked TCN Blocks với dilation tăng dần (1, 2, 4, ...)
        tcn_layers = []
        in_ch = gcn_hidden
    ...
    def forward(self, x):
        ...
        # --- Spatial: GCN xử lý từng time step ---
        x_flat = x.view(B * T, N, F_in)
        h = F.relu(self.gcn1(x_flat, self.A_norm))
        h = self.dropout(h)
        h = F.relu(self.gcn2(h, self.A_norm))
        h = self.dropout(h)

        # --- Temporal: TCN xử lý từng node ---
        h = h.view(B, T, N, -1)                     # (B, T, N, gcn_hidden)
        h = h.permute(0, 2, 1, 3).contiguous()      # (B, N, T, gcn_hidden)
        h = h.view(B * N, T, -1)                     # (B*N, T, gcn_hidden)
...
def run_training():
    ...
    print(f"TRAINING GCN-TCN (Steps={CFG.HORIZON}, Future={CFG.PRED_MINUTES} mins)")
