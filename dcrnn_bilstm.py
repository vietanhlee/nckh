...
class Config:
    ...
    def FULL_SAVE_PATH(self):
        ...
        return os.path.join(self.SAVE_DIR, f"model_DCRNN_BiLSTM_{self.HORIZON}steps.pth")
...
class BiLSTMTemporalLayer(nn.Module):
    """
    Bidirectional LSTM temporal learning layer
    """
...
class DCRNNBiLSTMBlock(nn.Module):
    """
    Spatio-Temporal block: BiLSTM -> Diffusion Graph Conv -> BiLSTM
    """
...
class DCRNN_BiLSTM_Model(nn.Module):
    """
    DCRNN model redesigned to use Bidirectional LSTMs
    """
    def __init__(self, num_nodes, in_feat, block_hidden, num_blocks, T_in,
        ...
        for _ in range(num_blocks):
            blocks.append(DCRNNBiLSTMBlock(c_in, block_hidden, num_nodes, K, dropout))
            c_in = block_hidden
...
def run_training():
    ...
    print(f"TRAINING DCRNN-BiLSTM (Steps={CFG.HORIZON}, Future={CFG.PRED_MINUTES} mins)")
