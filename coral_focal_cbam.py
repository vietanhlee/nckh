"""
coral_focal_cbam.py — Phân loại mức độ tắc nghẽn giao thông
Phiên bản 2: ConvNeXt-Tiny + CBAM Attention + CORAL Ordinal Focal Loss + Grad-CAM

Kiến trúc:
  Input (B, 3, 256, 512)
  → ConvNeXt-Tiny backbone → f32 (B, 768, 8, 16)
  → CBAM (Channel + Spatial Attention) → refined f32
  → Global Average Pooling → (B, 768)
  → CORAL ordinal head → 4 logits → Sigmoid → mức 1-5

Sử dụng Grad-CAM (tự cài đặt, KHÔNG cần thư viện ngoài) để
kiểm chứng model nhìn vào xe hay background.
"""
...
def tune_thresholds_by_accuracy(y_true, y_probs, initial=None, grid=None, max_passes=3):
    """Tune per-head thresholds to maximize overall multiclass accuracy."""
...
# ============================================================
# Model — CBAM + ConvNeXt-Tiny
# ============================================================

class ChannelAttention(nn.Module):
    """Channel Attention (SE-style): chọn lọc kênh feature quan trọng."""
...
class SpatialAttention(nn.Module):
    """Spatial Attention: tạo mask suppress background, amplify foreground."""
...
class CBAM(nn.Module):
    ...
    Channel Attention → Spatial Attention, tuần tự.
    Suppress background (bầu trời, biển hiệu), amplify foreground (xe cộ).
    """
...
class TrafficCongestionNetCBAM(nn.Module):
    """ConvNeXt-Tiny + CBAM + CORAL ordinal regression.

    Forward output: logits tensor (B, C-1) — KHÔNG trả dict.
    Dùng Grad-CAM bên ngoài để visualize.
    """
    ...
    def __init__(self, num_classes=NUM_CLASSES, hidden_dim=256, dropout=0.3, pretrained=True):
        ...
        # CBAM Attention
        self.cbam = CBAM(feat_dim, reduction=16, kernel_size=7)
    ...
    def forward(self, x):
        x = self.stem(x)
        x = self.stages(x)
        x = self.norm_pre(x)   # (B, 768, H/32, W/32)
...
# ============================================================
# Simple Grad-CAM (KHÔNG cần thư viện ngoài)
# ============================================================

class SimpleGradCAM:
    """Grad-CAM tự cài đặt cho ordinal regression.

    Hook vào target_layer để lấy activation và gradient,
    tính heatmap = ReLU(Σ_c w_c * A_c) với w_c = GAP(gradients).

    Dùng để kiểm chứng model nhìn vào xe hay background.
    """
    ...
    def generate(self, input_tensor):
        ...
        Returns:
            cam: Tensor (B, 1, H', W') ∈ [0, 1]
            logits: Tensor (B, C-1)
        """
        self.model.zero_grad()
        logits = self.model(input_tensor)

        # Backprop từ sum(sigmoid(logits)) = "congestion score" tổng thể
        score = torch.sigmoid(logits).sum()
        score.backward()

        # Grad-CAM: GAP(gradients) × activations, rồi ReLU
        weights = self.gradients.mean(dim=(2, 3), keepdim=True)   # (B, C, 1, 1)
        cam = (weights * self.activations).sum(dim=1, keepdim=True)  # (B, 1, H', W')
        cam = F.relu(cam)

        # Normalize từng sample về [0, 1]
        b = cam.size(0)
        for i in range(b):
            c_min = cam[i].min()
            c_max = cam[i].max()
    ...
    def remove_hooks(self):
        """Gỡ hooks khi không cần nữa (tránh memory leak)."""
...
def get_split_indices(dataset, val_ratio=0.2, seed=42):
    ...
    for c in range(num_classes):
        ...
        if n <= 1:
            ...
            n_val = max(1, min(int(round(val_ratio * n)), n - 1))
...
def get_train_transform():
    """Augmentation an toàn: KHÔNG RandomCrop/RandomResizedCrop."""
...
def train_epoch(epoch, model, loader, loss_func, optimizer, scheduler, device, scaler):
    ...
    for i, (images, labels) in enumerate(iterator):
        ...
        with torch.cuda.amp.autocast(enabled=scaler.is_enabled()):
            logits = model(images)  # Tensor, không phải dict
...
# ============================================================
# Visualization — Grad-CAM Heatmap
# ============================================================

def visualize_gradcam(model, dataset, indices, device, thresholds=None,
                      num_images=10, save_path='gradcam_heatmap_samples.png'):
    """Vẽ Grad-CAM heatmap chồng lên ảnh gốc.

    Nếu điểm nóng trùng vùng xe → model đúng.
    Nếu điểm nóng ở bầu trời/biển hiệu → model sai.
    """
    ...
    # Hook vào stage cuối của ConvNeXt (trước CBAM) để xem backbone nhìn đâu
    gradcam = SimpleGradCAM(model, model.norm_pre)
    ...
    for i in range(num_to_show):
        ...
        # Upsample Grad-CAM về kích thước ảnh gốc
        heat = F.interpolate(
        ...
        axes[i, 1].set_title(f"Grad-CAM — Dự đoán: {pred + 1}", fontsize=10)
        axes[i, 1].axis('off')

    # Gỡ hooks
    gradcam.remove_hooks()

    plt.suptitle("Kiểm chứng: Model nhìn vào xe hay background? (Grad-CAM)", fontsize=14)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.show()
    print(f"Đã lưu Grad-CAM heatmap tại: {save_path}")
...
def main():
    ...
    print(f"Model: ConvNeXt-Tiny + CBAM + Grad-CAM")
    ...
    # ── 5. Model ──────────────────────────────────────────
    model = TrafficCongestionNetCBAM(
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if acc_tuned > best_acc:
            best_acc = acc_tuned
            save_path = f"best_model_convnext_cbam_{acc_tuned:.2f}.pth"
    ...
    plt.tight_layout()
    plt.savefig('training_metrics_convnext_cbam.png')
    plt.show()
    ...
    if os.path.exists(save_path):
        ...
        visualize_gradcam(
            ...
            save_path='gradcam_heatmap_samples.png'
        )
    else:
