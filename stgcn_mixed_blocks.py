...
# ============================================================
#  MÔ HÌNH: STGCN MIXED BLOCKS (2 Khối GLU Nguyên bản + 1 Khối Attention)
# ============================================================
# Kiến trúc kết hợp linh hoạt (Hybrid Block-Level Architecture):
#   - Block 1: STGCN Block nguyên bản (GLU 1 -> ChebNet -> GLU 2)
#   - Block 2: STGCN Block nguyên bản (GLU 1 -> ChebNet -> GLU 2)
#   - Block 3: STGCN Block Attention (Attn 1 -> ChebNet -> Attn 2)
#   - Final Attention: Lớp Multi-Head Temporal Attention ở cuối trước Conv1D
# ============================================================
...
class Config:
    ...
    USE_FINAL_ATTENTION = True     # Lớp Temporal Attention ở CUỐI mô hình
...
class TemporalConvLayer(nn.Module):
    """
    Temporal Gated Convolution Layer (TCN block trong STGCN nguyên bản)
    """
...
# --- KHỐI 1 & 2: STGCN BLOCK NGUYÊN BẢN (GLU 1 -> ChebNet -> GLU 2) ---

class STGCNBlockOriginal(nn.Module):
    """
    Khối STGCN Nguyên bản: GLU 1 -> Spatial ChebNet -> GLU 2
    """
    def __init__(self, in_channels, out_channels, num_nodes, cheb_K=3, dropout=0.25):
        super().__init__()
...
# --- KHỐI 3: STGCN BLOCK ATTENTION (Attn 1 -> ChebNet -> Attn 2) ---

class STGCNBlockAttn(nn.Module):
    """
    Khối STGCN Block Attention: Attn 1 -> Spatial ChebNet -> Attn 2
    """
    ...
    def forward(self, x, L_tilde):
        ...
        h = h_attn1.view(B, N, T, C).permute(0, 3, 1, 2)
        ...
        h = h_attn2.view(B, N, T, C).permute(0, 3, 1, 2)
...
# ============================================================
#  MIXED MODEL PIPELINE (2 Original GLU Blocks + 1 Attention Block)
# ============================================================

class STGCN_Mixed_Model(nn.Module):
    """
    Mô hình STGCN Mixed Blocks:
      - Block 1: STGCNBlockOriginal (GLU 1 -> ChebNet -> GLU 2)
      - Block 2: STGCNBlockOriginal (GLU 1 -> ChebNet -> GLU 2)
      - Block 3: STGCNBlockAttn (Attn 1 -> ChebNet -> Attn 2)
      - Final Attention & Conv1D Output Projection
    """
    def __init__(self, num_nodes, in_feat, block_hidden, T_in,
        ...
        # Block 1 & Block 2: Nguyên bản GLU
        self.block1 = STGCNBlockOriginal(in_feat, block_hidden, num_nodes, cheb_K, dropout)
        self.block2 = STGCNBlockOriginal(block_hidden, block_hidden, num_nodes, cheb_K, dropout)

        # Block 3: Attention Block
        self.block3 = STGCNBlockAttn(block_hidden, block_hidden, num_nodes, cheb_K, num_heads, dropout)
...
def plot_training_history(train_losses, val_losses, train_maes, val_maes, save_dir="plots", use_wandb=False, tag="stgcn_mixed_blocks"):
...
def run_training():
    ...
    print(f"[Cấu hình] Mixed Blocks: 2 GLU Blocks + 1 Attention Block | Hidden={CFG.BLOCK_HIDDEN} | "
