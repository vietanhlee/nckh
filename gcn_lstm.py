...
class Config:
    ...
    def FULL_SAVE_PATH(self):
        ...
        return os.path.join(self.SAVE_DIR, f"model_MultiStep_{self.HORIZON}steps_NoRes_NoOverlap.pth")
...
def load_timeseries_double_rolling(csv_path, node_list, window1 = 3, window2 = 5, step_minutes=5):
    ...
    # smooth_1 = pivot_1min.ewm(span=window1, adjust=False).mean()
...
class ImprovedGNN_LSTM(nn.Module):
    def __init__(self, num_nodes, in_feat, gcn_hidden, lstm_hidden,
                 lstm_layers, horizon, output_feat, A_norm=None, dropout=0.3):
        super().__init__()
        self.horizon = horizon
        ...
        self.lstm = nn.LSTM(gcn_hidden, lstm_hidden, num_layers=lstm_layers,
        ...
        self.proj_out = nn.Linear(lstm_hidden // 2, horizon * output_feat)
    ...
    def forward(self, x):
        ...
        lstm_in = h.view(B * N, T, -1)

        lstm_out, _ = self.lstm(lstm_in)
        last_hidden = lstm_out[:, -1, :]

        shared = self.proj_shared(last_hidden)
...
def run_training():
    ...
    print(f"TRAINING MULTISTEP NO-RESIDUAL (Steps={CFG.HORIZON}, Future={CFG.PRED_MINUTES} mins)")
