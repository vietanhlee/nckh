...
def load_timeseries_double_rolling(csv_path, node_list, window1=3, window2=5, step_minutes=5):
    ...
    pivot = df.pivot_table(index='Timestamp', columns='STT', values=['Car Count', 'Bike Count'], aggfunc='mean')
    pivot = pivot.swaplevel(0, 1, axis=1) # Swap so level 0 is Node (STT), level 1 is Feature
    
    desired_cols = pd.MultiIndex.from_product([node_list, ['Car Count', 'Bike Count']], names=['Node', 'Feature'])
