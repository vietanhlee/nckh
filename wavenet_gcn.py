...
class Config:
    ...
    # --- MODEL ---
    RESIDUAL_CHANNELS = 32     # Channels trong mỗi WaveNet block
    SKIP_CHANNELS     = 64     # Channels cho skip connections
    ADAPTIVE_EMB      = 16     # Embedding dimension cho adaptive adjacency
    DILATION_LIST     = [1, 2, 4, 8]   # Dilation rates cho WaveNet blocks
    DROPOUT           = 0.3
    ...
    def FULL_SAVE_PATH(self):
        ...
        return os.path.join(self.SAVE_DIR, f"model_GraphWaveNet_{self.HORIZON}steps.pth")
...
class GraphConvWN(nn.Module):
    """
    Graph Convolution đơn giản cho Graph WaveNet.
    Giống GCN baseline nhưng dùng cho cả fixed adj và adaptive adj.
    """
    ...
    def forward(self, x, A):
        ...
        Args:
            ...
            A: (N, N)       — adjacency matrix (fixed hoặc adaptive)
        Returns:
            (B, N, F_out)
        """
...
class GatedDilatedConv(nn.Module):
    ...
    Sử dụng cơ chế gating: output = tanh(filter) ⊙ sigmoid(gate)
    Causal convolution: padding bên trái để không nhìn thấy tương lai.
    Dilation tăng exponentially → receptive field lớn.
    """
    ...
    def forward(self, x):
        ...
        Args:
            x: (B, C, T)
        Returns:
            (B, C, T) — cùng kích thước nhờ causal padding
        """
        f = self.filter_conv(x)
        g = self.gate_conv(x)

        # Causal: cắt phần padding bên phải (tương lai)
        if self.causal_padding > 0:
            f = f[:, :, :-self.causal_padding]
            g = g[:, :, :-self.causal_padding]
...
class GraphWaveNet_Model(nn.Module):
    ...
    Kết hợp:
    - Adaptive Adjacency Matrix: học ma trận kề tự thích ứng từ node embeddings
      A_adp = softmax(ReLU(E1 · E2^T)), phát hiện hidden spatial dependencies
    - Gated Dilated Causal Convolution: WaveNet-style temporal processing
    - GCN trên cả fixed adj lẫn adaptive adj

    Pipeline mỗi layer:
      h → GatedDilatedConv → GCN(fixed) + GCN(adaptive) → residual + skip
    Output: tổng hợp skip connections → projection
    """
    def __init__(self, num_nodes, in_feat, residual_channels, skip_channels,
        ...
        # Adaptive adjacency: E1, E2 ∈ R^{N × d}
        ...
        # WaveNet blocks — mỗi block có dilation khác nhau
        ...
        for dilation in dilation_list:
            self.gated_convs.append(GatedDilatedConv(residual_channels, kernel_size=2, dilation=dilation))
    ...
    def forward(self, x):
        """
        Args:
            x: (B, T, N, F) — input từ DataLoader
        Returns:
            (B, Horizon, N, output_feat)
        """
        B, T, N, F_in = x.shape
        C = None   # will be set after input_proj
        ...
        for gated, skip_conv, res_conv, gcn_f, gcn_a, norm in zip(
            ...
            # --- Temporal: Gated Dilated Conv (per node) ---
            h_tn = h.permute(0, 2, 3, 1).reshape(B * N, C, T)  # (B*N, C, T)
            h_tn = gated(h_tn)                                   # (B*N, C, T)
            h = h_tn.view(B, N, C, T).permute(0, 3, 1, 2)       # (B, T, N, C)

            # --- Spatial: GCN trên fixed + adaptive adj (per timestep) ---
            h_sn = h.reshape(B * T, N, C)                        # (B*T, N, C)
            h_fixed = gcn_f(h_sn, self.A_norm)                   # (B*T, N, C)
            h_adp = gcn_a(h_sn, A_adp)                           # (B*T, N, C)
            h = (h_fixed + h_adp).view(B, T, N, C)

            # --- Skip connection ---
            h_skip = h.permute(0, 2, 3, 1).reshape(B * N, C, T) # (B*N, C, T)
            s = skip_conv(h_skip)                                 # (B*N, skip_channels, T)
            s = s.view(B, N, -1, T)                               # (B, N, skip_ch, T)
            if skip_total is None:
                skip_total = s
            else:
                skip_total = skip_total + s

            # --- Residual conv + connection ---
            h_res = h.permute(0, 2, 3, 1).reshape(B * N, C, T)  # (B*N, C, T)
            h_res = res_conv(h_res)                               # (B*N, C, T)
            h = h_res.view(B, N, C, T).permute(0, 3, 1, 2)       # (B, T, N, C)
            h = h + residual
        ...
        # skip_total: (B, N, skip_ch, T) → lấy timestep cuối cùng
...
def run_training():
    ...
    print(f"TRAINING Graph WaveNet (Steps={CFG.HORIZON}, Future={CFG.PRED_MINUTES} mins)")

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Device: {device}")

    # Load adjacency và normalize cho fixed GCN
    A_raw, nodes = load_adj_from_excel(CFG.ADJ_PATH)
    A_norm = normalize_adj_sym(A_raw)
    print(f"Fixed Adjacency loaded. Shape: {A_norm.shape}")
    print(f"Adaptive Adjacency sẽ được học từ node embeddings (dim={CFG.ADAPTIVE_EMB})")
