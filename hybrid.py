...
# ============================================================
#  CÁC CẢI TIẾN TRONG FILE NÀY (so với bản STGCN gốc)
# ============================================================
# 1. Gradient clipping           -> CFG.GRAD_CLIP_NORM
# 2. LR scheduler (ReduceLROnPlateau) -> CFG.USE_LR_SCHEDULER
...
class Config:
    ...
    # --- CẢI TIẾN 1: ỔN ĐỊNH HOÁ TRAINING ---
    GRAD_CLIP_NORM    = 5.0     # None để tắt clipping
    USE_LR_SCHEDULER  = True
    LR_SCHED_FACTOR   = 0.5
    LR_SCHED_PATIENCE = 10      # giảm LR nếu val_loss không cải thiện sau N epoch
    LR_SCHED_MIN_LR   = 1e-5

    # --- CẢI TIẾN 2: EMA TRỌNG SỐ (giống hiệu ứng SWA) ---
    USE_EMA    = True
    EMA_DECAY  = 0.995
...
# ============================================================
#  DATA UTILITIES (không đổi so với bản gốc)
# ============================================================
...
# ============================================================
#  DATASET (không đổi so với bản gốc)
# ============================================================
...
# ============================================================
#  MODEL: STGCN (+ tuỳ chọn GRU làm mượt thời gian)
# ============================================================
...
# ============================================================
#  LOSS: Huber + temporal-smoothness regularizer (tuỳ chọn)
# ============================================================

class HuberSmoothLoss(nn.Module):
    """
    Huber loss (giống loss gốc khi smooth_weight=0) + tuỳ chọn phạt chênh lệch
    giữa các bước dự đoán liên tiếp trong horizon (temporal-smoothness),
    giúp model tự học ra dự đoán mượt hơn mà không cần đổi kiến trúc.
    """
    def __init__(self, delta=1.0, smooth_weight=0.0):
        super().__init__()
    ...
    def forward(self, pred, target, x_last=None):
        ...
        if self.smooth_weight > 0 and pred.shape[1] > 1:
            diff = pred[:, 1:] - pred[:, :-1]     # chênh lệch giữa các bước horizon liên tiếp
            smooth_loss = torch.mean(diff ** 2)
            return base_loss + self.smooth_weight * smooth_loss
        return base_loss

# ============================================================
#  EMA: Exponential Moving Average của trọng số model
# ============================================================

class ModelEMA:
    """
    Duy trì một bản sao "mượt" của trọng số model (trung bình trượt theo epoch/step).
    Thường cho đường validation ổn định hơn và đôi khi tổng quát hoá tốt hơn,
    tương tự hiệu ứng của Stochastic Weight Averaging (SWA), nhưng không cần
    đổi lịch train hay kiến trúc.
    """
...
def train_one_epoch(model, loader, opt, loss_fn, device, scaler_obj, scaler_stats,
    ...
    for X, Y in pbar:
        ...
        if grad_clip_norm is not None:
            # Cần unscale trước khi clip gradient khi dùng AMP
...
def run_training():
    ...
    for ep in range(CFG.EPOCHS):
        ...
        # Đánh giá bằng trọng số EMA (nếu bật) -> đường val ổn định hơn,
        # và đây cũng chính là bản trọng số sẽ được lưu làm "best model".
        if ema is not None:
