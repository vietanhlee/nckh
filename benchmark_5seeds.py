...
from hybrid import STGCN_Model as Hybrid_STGCN_Model, Config as HybridConfig, HuberSmoothLoss, visualize_last_step, TemporalAttention, STGCNBlock
...
def train_single_seed(model_name, model, train_loader, val_loader, test_loader, cfg, device, seed, use_wandb=True, wandb_project="STGCN_NCKH_Benchmark"):
    ...
    if model_name == "TA-STGCN" and seed == 42:
        try:
            print("\n📸 Generating heatmap visualizations for TA-STGCN (Seed 42)...")
            visualize_last_step(model, test_loader, device, scaler_stats, cfg, node_list=[7, 266, 489, 89, 26, 32, 380, 365, 557])
        except Exception as e:
...
def run_benchmark():
    ...
    plot_pdf = os.path.join('plots', 'val_mae_benchmark.pdf')
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.savefig(plot_pdf, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"📈 Đã lưu biểu đồ đường cong Validation MAE vào: {plot_path} (và .pdf)")
    ...
    # ------------------ COMBINED FIGURE FOR MAE AND MAPE ------------------
    ...
    for model_name in main_models:
        ...
        # Plot MAE on ax1
        ax1.plot(x_steps, step_maes_mean, label=model_name, color=color, 
                 marker=marker, linewidth=linewidth, linestyle=linestyle, markersize=7)
        
        # Plot MAPE on ax2
        ax2.plot(x_steps, step_mapes_mean, label=model_name, color=color, 
                 marker=marker, linewidth=linewidth, linestyle=linestyle, markersize=7)

    # Styling for ax1 (MAE)
    ax1.set_xlabel('Prediction Horizon (Minutes Ahead)', fontsize=11, fontweight='bold')
    ax1.set_ylabel('Mean Absolute Error (MAE)', fontsize=11, fontweight='bold')
    ax1.set_title('MAE Growth across Prediction Horizons', fontsize=12, fontweight='bold')
    ax1.set_xticks(x_steps)
    ax1.set_xticklabels(x_labels)
    ax1.grid(True, linestyle='--', alpha=0.6)
    
    # Styling for ax2 (MAPE)
    ax2.set_xlabel('Prediction Horizon (Minutes Ahead)', fontsize=11, fontweight='bold')
    ax2.set_ylabel('Mean Absolute Percentage Error (MAPE %)', fontsize=11, fontweight='bold')
    ax2.set_title('MAPE Growth across Prediction Horizons', fontsize=12, fontweight='bold')
    ...
    fig.legend(handles, labels, bbox_to_anchor=(1.02, 0.95), loc='upper left', fontsize=10)
    
    plt.tight_layout()
    
    cmb_png_path = os.path.join('plots', 'error_by_horizon.png')
    cmb_pdf_path = os.path.join('plots', 'error_by_horizon.pdf')
    plt.savefig(cmb_png_path, dpi=300, bbox_inches='tight')
    plt.savefig(cmb_pdf_path, dpi=300, bbox_inches='tight')
    ...
    print(f"📉 Đã lưu biểu đồ sai số Combined Error by Horizon vào: plots/error_by_horizon.png (và .pdf)")
...
class Ablation_TA_STGCN_Model(nn.Module):
    def __init__(self, num_nodes, in_feat, block_hidden, num_blocks, T_in,
        ...
        blocks = []
        c_in = block_hidden if self.position == 'before' else in_feat
    ...
    def forward(self, x):
        ...
        if self.position == 'none' or not self.use_temporal_attention:
            ...
            for i, block in enumerate(self.blocks):
                h = block(h, self.L_tilde)
                if i == mid_idx - 1: # Apply after 1st block (khối 1)
                    h = self.apply_attn(h)

        elif self.position == 'middle_late':
            mid_idx = len(self.blocks) // 2
            for i, block in enumerate(self.blocks):
                h = block(h, self.L_tilde)
                if i == mid_idx: # Apply after 2nd block (khối 2)
                    h = self.apply_attn(h)
