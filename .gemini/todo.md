...
- [x] Cấu trúc và hoàn thiện toàn bộ bài báo khoa học tiếng Anh chuyên nghiệp (LaTeX IEEEtran) trong thư mục `paper/`:
  ...
  - **Khắc phục lỗi `AttributeError: 'Config' object has no attribute 'WEIGHT_DECAY'` trong `run_ablation_study.py`**: Đã sử dụng `getattr(cfg, 'WEIGHT_DECAY', 1e-4)`, `getattr(cfg, 'LR_SCHED_FACTOR', 0.5)` và `getattr(cfg, 'LR_SCHED_PATIENCE', 10)` giúp script tương thích 100% với class `Config` trong `hybrid.py`.
