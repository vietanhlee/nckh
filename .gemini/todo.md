...
- [x] Cấu trúc và hoàn thiện toàn bộ bài báo khoa học tiếng Anh chuyên nghiệp (LaTeX IEEEtran) trong thư mục `paper/`:
  ...
  - **Bổ sung Sơ đồ Kiến trúc ConvNeXt-Tiny Counting Model**: Đã chèn tệp hình ảnh **`ConvNext_tiny_coutin.png`** (Figure~\ref{fig:convnext_counting_arch}) minh họa đầy đủ luồng xử lý từ Patchify stem, 4 khối ConvNeXt $7 \times 7$ depthwise conv đến đầu ra MLP Regression vào [methodology.tex](file:///g:/nckh/paper/sections/methodology.tex).
